...
namespace Project_WPR.Tests
{
    public class VehicleControllerTests
    {
        ...
        public void GetVehicles_ReturnsAllVehicles()
        {
            ...
            // Act
            var result = _controller.GetVehicles(0);

            // Assert
            var okResult = Assert.IsType<OkObjectResult>(result.Result);
            var value = okResult.Value;
            var carsList = value.GetType().GetProperty("cars").GetValue(value) as List<data.Car>;
            var campersList = value.GetType().GetProperty("campers").GetValue(value) as List<data.Camper>;
            var caravansList = value.GetType().GetProperty("caravans").GetValue(value) as List<data.Caravan>;

            Assert.NotNull(carsList);
            Assert.NotNull(campersList);
            Assert.NotNull(caravansList);

            var vehicles = carsList.Cast<data.Vehicle>()
                .Union(campersList)
                .Union(caravansList);
